...
class Validator:
    @staticmethod
    def check_bool(setting: SettingOption, errors: list):
        value = setting.value
    ...
    @staticmethod
    def validate(s: SettingOption) -> bool:
        ...
        if s.validation is not None:
            ...
            if len(errors) > 0:
                ...
                cprint('\n'.join(map(SettingOption.dash, errors)), 'red')
                return False
        return True
...
def load_settings():
    ...
    for k in settings.keys():
        setting: SettingOption = settings[k]
